...
namespace PickToLight.Core {
    public class Controller {
        ...
        private string Write(byte ccbLength, byte subCommand, byte? subNode = null, byte[] data = null, string result = "") {
            List<byte> ccbList = new List<byte> { ccbLength, 0x00, MessageType, 0x00, 0x00, 0x00, subCommand };
            if (subNode != null) ccbList.Add((byte)subNode);
            if (data != null) ccbList.AddRange(data);
            byte[] ccb = ccbList.ToArray();
            try {
        ...
        public string SetFlashingTimeInterval(byte subNode = 0xFC, FlashingTimeInterval interval = FlashingTimeInterval.QuarterSecond) {
            byte[] data = new byte[] { 0x00, (byte)interval };
        ...
        public string SetAvailableDigitsForCounting(byte subNode = 0xFC, int availableDigits = 6) {
            byte[] data = new byte[] { (byte)availableDigits };
        ...
        public string SetColor(Color color, byte subNode = 0xFC, bool shouldStore = false) {
            byte[] data = shouldStore ? new byte[] { 0x00, (byte)color, 0x55 } : new byte[] { 0x00, (byte)color };
        ...
        public string SetValidDigitsForCounting(byte subNode = 0xFC, ValidDigitsConfig config = null) {
            ...
            byte[] data = new byte[] { 0x01, validDigits };
        ...
        public string ConfigureMode(byte subNode = 0xFC, PickTagModeConfig config = null, bool shouldStore = false) {
            ...
            byte[] data = shouldStore ? new byte[] { 0x02, mode, 0x55 } : new byte[] { 0x03, mode };
        ...
        public string SetBlinkingTimeInterval(byte subNode = 0xFC, BlinkingTimeInterval interval = BlinkingTimeInterval.QuarterSecond) {
            byte[] data = new byte[] { 0x04, (byte)interval };
        ...
        public string SetDigitsBrightness(byte subNode = 0xFC, DigitsBrightnessConfig config = null) {
            ...
            byte[] data = new byte[] { 0x06, brightness };
        ...
        public string ConfigureSpecialFunctionOne(byte subNode = 0xFC, SpecialFunctionOneConfig config = null, bool shouldStore = false) {
            ...
            byte[] data = shouldStore ? new byte[] { 0x0A, configuration, 0x55 } : new byte[] { 0x0A, configuration };
        ...
        public string ConfigureSpecialFunctionTwo(byte subNode = 0xFC, SpecialFunctionTwoConfig config = null, bool shouldStore = false) {
            ...
            byte[] data = shouldStore ? new byte[] { 0x0F, configuration, 0x55 } : new byte[] { 0x0F, configuration };
        ...
        public string SetNodeAddress(byte oldSubNode, byte newSubNode) {
            byte[] data = new byte[] { 0x40, 0x1B, 0x1B, 0x10, newSubNode };
        ...
        public static string ReadControl(byte[] cbb) {
            ...
            byte[] data = null;
            if (cbb.Length > 8) {
                data = new byte[cbb.Length - 8];
                Array.Copy(cbb, 8, data, 0, cbb.Length - 8);
            }
            switch (subCommand) {
                case 0x06:
                    return ReadConfirmationButtonPressing(subNode, data);
                case 0x07:
                    return ReadShortageButtonPressing(subNode, data);
                case 0x09:
                    return ReadConnectedPickTagsNodeAddresses(data);
                case 0x0A:
                    return ReadTimeout(subNode);
                case 0x0B:
                    return ReadMalfunction(subNode);
                case 0x0C:
                    return ReadIllegalCommand(subNode);
                case 0x0D:
                    return ReadConfirmationButtonLocked(subNode);
                case 0x0E:
                    return ReadOldDeviceResettedOrConnected(subNode);
                case 0x0F:
                    return ReadQuantity(subNode, data);
                case 0xFA:
                    return ReadPickTagModelName(subNode, data);
                case 0xFC:
                    return ReadDeviceDetailConfiguredInformation(subNode/*, data*/);
                /*case 0x64:
                    return ReadSpecial(subNode, data);*/
                default:
                    return "This return message is not implemented yet";
            }
        }
        ...
        private static string ReadConnectedPickTagsNodeAddresses(byte[] data) {
            byte[] dataOfNodeAddresses = new byte[data.Length - 3];
            Array.Copy(data, 3, dataOfNodeAddresses, 0, data.Length - 3);
            List<byte> nodeAddresses = new List<byte>();
